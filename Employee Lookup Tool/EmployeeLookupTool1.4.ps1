...
function Search-ADUser {
    ...
    try {
        ...
        $script:domainUsername = "$domainName\$adminUsername" 
        
        # Prompt for password when needed
        $script:credential = Get-Credential -Message "Enter password for $script:domainUsername" -UserName $script:domainUsername
        ...
        foreach ($server in $servers) {
            try {
                ...
                Write-Host "Testing connection to $server with $script:domainUsername"
                Get-ADDomain -Server $server -Credential $script:credential -ErrorAction Stop | Out-Null
        ...
        if ($user) {
            ...
            $script:CurrentDomain = $domainName
            $script:CurrentCredential = $script:credential
            $script:CurrentDC = $dcServer
        }
        else {
...
Function Get-ADUC {
    ...
    $credString = $script:domainUsername
    Write-Host "Launching ADUC for domain: $domain with username: $script:domainUsername"
...
function Open-ADUC {
    if ($script:CurrentUser -and $script:CurrentDomain) {
        try {
            # Get the domain info
            $domainName = $script:CurrentDomain
            ...
            if ($script:credential) {
                ...
                Get-ADUC -domain $domainName -userName $adminUsername -credential $script:credential
            }
        }
        catch {
